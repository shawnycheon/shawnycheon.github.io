function [nx, ny, textbounds, wordbounds] = DrawFormattedText_CAPS(win, tstring, sx, sy, color, wrapat, scalex, scaley, rotatedeg, vSpacing, righttoleft, winRect)
% [nx, ny, textbounds, wordbounds] = DrawFormattedText(win, tstring [, sx][, sy][, color][, wrapat][, scalex][, scaley][, rotatedeg][, vSpacing][, righttoleft][, winRect])
%
...
%
% 17/07/06 : J.J.Lee added scalex, scaley, and rotatedeg optional flag.
%            This option can scale and rotate the text.
%            (Unit of rotation is degree)
%
...
% No horizontal scaling by default:
if nargin < 7 || isempty(scalex)
    scalex = 1;
end

% No vertical scaling by default:
if nargin < 8 || isempty(scaley)
    scaley = 1;
end

% No rotation by default:
if nargin < 9 || isempty(rotatedeg)
    rotatedeg = 0;
end
...
while ~isempty(tstring)
    ...
    if ~isempty(curstring) && noclip
        ...
        % Need bounding box?
        if xcenter || scalex || scaley || rotatedeg || rjustify
        ...
        if scalex || sclaey || rotatedeg
            ...
            if ~isempty(scalex)
                Screen('glScale', win, scalex, 1, 1);
            end
            
            if ~isempty(scaley)
                Screen('glScale', win, 1, scaley, 1);
            end
                
            if ~isempty(rotatedeg)
                Screen('glRotate', win, rotatedeg);
            end
